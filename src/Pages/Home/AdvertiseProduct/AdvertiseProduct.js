  ...
  const { data: advertiseProduct = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        "https://laptop-planet-server.vercel.app//advertise-product"
      );
    ...
    <>
      {advertiseProduct.length > 0 && (
        <section className="py-20 bg-[#cbbdcb22]">
          <h2 className="text-2xl md:text-4xl font-extrabold font-mono text-center mb-20">
            Advertise Products
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5  container">
            {advertiseProduct.map((item) => (
              <AdvertiseCard
                key={item._id}
