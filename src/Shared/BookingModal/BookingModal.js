...
const BookingModal = ({ singleProduct, setSingleProduct }) => {
  ...
  const handleBookLaptop = () => {
    fetch("https://laptop-planet-server.vercel.app//bookings", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        if (data.acknowledged) {
          fetch(`https://laptop-planet-server.vercel.app//products/${_id}`, {
            method: "PUT",
          })
            .then((res) => res.json())
            .then((data) => {
      ...
      <div className="modal">
        <div className="modal-box md:w-1/3 w-full md:mx-0 mx-1 max-w-5xl">
          <h3 className="text-2xl text-center">
            Welcome for coming to booking this Laptop
          </h3>
          <form>
            <input
              type="text"
          ...
          <div className="modal-action">
            <label onClick={handleBookLaptop} className="btn ">
              Confirm
            </label>
            <label htmlFor="booking-modal" className="btn btn-outline">
              Cancel
            </label>
          </div>
        </div>
