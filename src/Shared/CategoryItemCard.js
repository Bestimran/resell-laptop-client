...
const CategoryItemCard = ({ product, setSingleProduct }) => {
  ...
  const handleWishList = (id) => {
    ...
    fetch(`https://laptop-planet-server.vercel.app//wishlist`, {
      method: "POST",
      headers: {
      ...
      <div className="card-body">
        <div className="px-4">
          <div className=" flex justify-between">
            <h2 className="card-title md:text-3xl text-2xl font-bold font-mono">
              Brand: {product.categoryName}
            </h2>
            <label
              htmlFor="generic-modal"
              className="md:text-xl btn btn-sm md:btn-xl md:btn-outline"
            >
              wishList
            </label>
          </div>
          <div className=" text-xl  font-semibold">
            <h2 className="card-title md:text-3xl text-2xl font-semibold font-mono">
          ...
          <h3 className="text-xl ">
            <span className="font-bold">Seller </span>:{product.sellerName}{" "}
            {product?.verified ? (
              <span className=" border rounded-full bg-green-700 text-white px-1">
                verified
              </span>
            ) : (
              <span className=" border rounded-full bg-green-700 text-white px-1">
        ...
        <GenericModal
          ...
          message={
            "if you add this item wishList you can buy now go for your wishList dashboard"
          }
          product={product}
